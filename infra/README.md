...
- `.env.example` — 환경변수 템플릿. **바로 도는 고정 자격증명이 채워져 있다**(`appuser`/`appuser`, `admin`/`admin`, 토큰 5개) — 공개된 값이니 시험용으로만. 접속 주소까지 채워져 있어 `.env`로 복사하면 그대로 돈다 — 모델 연결(`LLM_API_BASE`·`LLM_API_KEY`)만 정한다(실제 `.env`는 커밋 금지)
